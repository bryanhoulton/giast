...
export class Parser {
  ...
  private parseUINode(): UINode {
    if (this.match(TokenType.BUTTON)) {
      ...
      throw new ParseError(
        `Expected UI component, got: ${this.peek().value}`,
        this.peek()
      );
    }
  }

  private parseButton(): UINode {
    this.consume(TokenType.LEFT_BRACE, "Expected '{' after 'Button'");
    this.skipNewlines();

    let text = "";
    let onClick: Stmt | Stmt[] | undefined;
  ...
  private parseOnClick(): Stmt | Stmt[] {
